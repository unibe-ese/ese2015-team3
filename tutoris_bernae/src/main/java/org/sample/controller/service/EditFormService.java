...
public class EditFormService{
    ...
    public EditForm saveFrom(EditForm editForm) throws InvalidUserException{
		...
		if(!usernameAvailable(email)) throw new InvalidUserException("Your email must be unique");
		user.setEmail(email);
		...
		if(!emailAvailable(username)) throw new InvalidUserException("Your username must be unique");
